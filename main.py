...
layout = [[sg.Text('         IoT-ভিত্তিক ফসলের সুপারিশ সিস্টেম', font=("Helvetica", 30), text_color = 'blue')],                                                    # Defining the layout of the Graphical User Interface. It consist of some text, Buttons, and blanks to take Input.
         ...
         [sg.Text('মাটিতে নাইট্রোজেনের অনুপাত                :', font=("Helvetica", 20)), sg.Input(font=("Helvetica",20), size = (20,1) )],
...
while True: 
	...
	if predict1 == 0:                                                                                              # Above we have converted the crop names into numerical form, so that we can apply the machine learning model easily. Now we have to again change the numerical values into names of crop so that we can print it when required. 
		...
		crop_name = 'Blackgram(কালো জিরা)'
	elif predict1 == 3:
		crop_name = 'Chickpea(ছোলা)'
	elif predict1 == 4:
		crop_name = 'Coconut(নারকেল)'
	elif predict1 == 5:
		...
		crop_name = 'Mothbeans(মথ সিম)'
	elif predict1 == 14:
		crop_name = 'Mungbeans(মুগ মটরশুটি)'
	elif predict1 == 15:
		crop_name = 'Muskmelon(খরবুজা)'
	elif predict1 == 16:
	...
	window['-OUTPUT1-'].update('এই জমির জন্য সবচেয়ে ভালো ফসল  : ' + crop_name )                                     # Suggesting the best crop after prediction.
	speak("এই জমির জন্য সবচেয়ে ভালো ফসল  " + crop_name)                                                         # Speaking the name of the predicted crop.
